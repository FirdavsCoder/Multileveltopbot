...
async def send_message(chat_id, user_id, message_id, reply_markup=None, mail_type='oddiy') -> bool:
    try:
        if mail_type == 'oddiy':
            if reply_markup:
        ...
        return await send_message(chat_id, user_id, message_id, reply_markup, mail_type)  # Recursive call
        ...
        if res['status']:
            ...
            if not user:
                ...
                for x in user:
                    ...
                    send_post_result = await send_message(chat_id=user_id, user_id=x['user_id'], message_id=message_id,
                                                          reply_markup=reply_markup, mail_type=mail_type)

                    if send_post_result:
                        send += 1
                    else:
                        not_send += 1
                        await db.update_user_status(status='passive', user_id=x['user_id'])

                    await db.update_mailing_results(send=send, not_send=not_send, offset=x['id'], id=id)
